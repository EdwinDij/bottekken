const {
  ...
  module.exports = {
    data: new SlashCommandBuilder()
      .setName("faq-delete")
      .setDescription(
        "Affiche les questions de la F.A.Q. Sélectionnez celle que vous souhaitez supprimer avec son numéro."
      ),
      ...
      try {
        ...
        db.all("SELECT * FROM Questions", function (err, questionRows) {
          ...
          // Vérifier si l'utilisateur est un administrateur
            // if (!interaction.member.permissions.has("Modérateur") || !interaction.member.permissions.has("Helper")) {
            //     interaction.reply("Vous devez être administrateur pour supprimer une question.");
            //     return;
            // }
          ...
          let faqMessage = "Sélectionnez la question que vous souhaitez supprimer:\n";
          ...
          interaction.client.once("interactionCreate", async (buttonInteraction) => {
            ...
            db.run("DELETE FROM Questions WHERE id = ?", buttonId, function (err) {
              if (err) {
                console.error(err);
                buttonInteraction.reply("Une erreur s'est produite lors de la suppression de la question.");
                return;
              }
  
              // Supprimer les réponses liées à la question
              db.run("DELETE FROM Reponses WHERE question_id = ?", buttonId, function (err) {
                if (err) {
                  console.error(err);
                  buttonInteraction.reply("Une erreur s'est produite lors de la suppression de la réponse.");
                  return;
                }
  
                buttonInteraction.reply("La question et sa réponse ont été supprimées avec succès.");
              });
            });
          });
        });
        ...
        console.error(error);
        interaction.reply("Une erreur s'est produite lors de la suppression de la question.");
      }
    },
  };
